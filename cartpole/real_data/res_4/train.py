...
learned_res = np.load('../res_3/learned.npy', allow_pickle=True).item()
...
true_theta = vertcat(vec(learned_A),
                     vec(learned_B),
                     vec(learned_C),
                     vec(learned_D),
                     vec(learned_E),
...
learner = cartpole_class.cartpole_learner(n_state, n_control, n_lam=n_lam, stiffness=1.)
