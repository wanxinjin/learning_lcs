...
for iter in range(max_iter):
    ...
    opt_lam_phi_batch, opt_mu_batch, opt_loss_batch, opt_lam_batch, opt_phi_batch = \
        vn_learner.compute_lam2(batch_x=x_mini_batch,
                               batch_u=u_mini_batch,
                               batch_lam=lam_mini_batch,
                               curr_theta=vn_curr_theta)

    # compute the gradient
    vn_loss, dtheta, dyn_loss, lcp_loss = vn_learner.gradient2(batch_x=x_mini_batch,
                                                              batch_u=u_mini_batch,
                                                              batch_lam=lam_mini_batch,
                                                              opt_batch_lam_phi=opt_lam_phi_batch,
                                                              opt_batch_mu=opt_mu_batch,
                                                              curr_theta=vn_curr_theta)
