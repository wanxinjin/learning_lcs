...
qp_learner = lcs_test.QPState_learner(n_state=n_state, n_control=n_control, n_lam=n_lam)
qp_optimizier = opt.Adam()
qp_optimizier.learning_rate = 1e-4
...
for iter in range(max_iter):
    ...
    # solve lam
    nextx_mini_sol, lam_mini_sol = qp_learner.qpSol(qp_curr_theta, x_mini_batch, u_mini_batch)

    # gradient
    loss, dtheta, res = qp_learner.gradient(qp_curr_theta, x_mini_batch, u_mini_batch, nextx_mini_sol,
                                            lam_mini_sol,
                                            goal_x_mini_batch)
