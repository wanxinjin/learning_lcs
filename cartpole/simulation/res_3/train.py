...
for k in range(5000):
    ...
    # compute the lam based on current lcp
    lam_opt_mini_batch, lcp_loss_mini_batch = learner.compute_lambda(x_mini_batch, u_mini_batch, curr_theta_lcp)

    # do the regression for the dynamics
    ...
    # store and update
    loss_trace += [loss]
    theta_lcp_trace += [curr_theta_lcp]
    # curr_theta=curr_theta-1e-3*dtheta
    # curr_theta = optimizier.step(curr_theta, dtheta)
    curr_theta_lcp = optimizier.step(curr_theta_lcp, dtheta_lcp)
