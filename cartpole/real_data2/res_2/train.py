...
plt.scatter(train_x, train_y, s=80, alpha=0.3)
pred_x, pred_y = [], []
...
train_x = train_x_batch[:, plot_x_indx]
train_y = train_x_next_batch[:, plot_y_indx]
plt.scatter(train_x, train_y, s=0, alpha=0.3)
pred_x, pred_y = [], []
...
alp_A = np.array([[0.2, 0.04122607, 1., 0.],
                  [0.2, 0.32651998, 0., 1.]])
...
curr_theta = 1*true_theta + 0.0 * np.random.randn(learner.n_theta)
mini_batch_size = 500
loss_trace = []
theta_trace = []
optimizier = opt.Adam()
optimizier.learning_rate = 5e-4
...
for k in range(10000):
    ...
    # learner.differetiable(epsilon=1.)
