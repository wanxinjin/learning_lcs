...
class LCS_VN:
    ...
    def diff(self, gamma=1e-2, epsilon=1., F_ref=0., w_F=0., C_ref=0., w_C=0.):
        ...
        # Q_dyn = np.diag([1, 1, 1, 10])
        # dyn_loss = dot(dyn - x_next, Q_dyn @ (dyn - x_next))
        ...
        # dl_dtheta = jacobian(total_loss, self.theta).T
        # ddl_ddtheta = jacobian(dl_dtheta, self.theta)
        # ddl_dthetadz = jacobian(dl_dtheta, lam_phi)
        # dl_dz = jacobian(total_loss, lam_phi).T
        # ddl_ddz = jacobian(dl_dz, lam_phi)
        # hess_loss = ddl_ddtheta - ddl_dthetadz @ inv(diag(dl_dz) + diag(lam_phi) @ ddl_ddz) @ diag(
        #     lam_phi) @ ddl_dthetadz.T
        #
        # hess_fn = Function('hess_fn', [data_pair, lam_phi, self.theta], [hess_loss])
        #
    ...
    def step(self, batch_x, batch_u, batch_x_next, current_theta):
        ...
        mean_x_next_norm = (batch_x_next * batch_x_next).sum(axis=1).mean()
        mean_pred_loss_relative = mean_dyn_loss / mean_x_next_norm

        return mean_loss, mean_dtheta, mean_dyn_loss, mean_lcp_loss, mean_pred_loss_relative
...
class LCS_Penalty:
    ...
    def diff(self, epsilon=1., F_ref=0., w_F=0., C_ref=0., w_C=0.):
        ...
        # lcp loss
        dist = self.D @ x + self.E @ u + self.F @ lam + self.lcp_offset
        lcp_loss = dot(lam * lam, phi * phi) + dot(phi - dist, phi - dist)
        ...
        lam_phi = vertcat(lam, phi)
        opts = {'ipopt.print_level': 0, 'ipopt.sb': 'yes', 'print_time': 0}
        prob = {'f': total_loss, 'x': lam_phi, 'p': data_theta}
        ...
        dyn_loss_plus = dyn_loss + \
                        w_C * dot(vec(DM(C_ref)) - vec(self.C), vec(DM(C_ref)) - vec(self.C)) + \
                        w_F * dot(vec(DM(F_ref)) - vec(self.F), vec(DM(F_ref)) - vec(self.F))
        self.loss_fn = Function('loss_fn', [data_pair, lam_phi, self.theta],
                                [jacobian(total_loss, self.theta).T, dyn_loss_plus, lcp_loss])
    ...
    def step(self, batch_x, batch_u, batch_x_next, current_theta):
        ...
        sol = self.solver(lbx=0., p=data_theta_batch.T)
        loss_batch = sol['f'].full()
        lam_phi_batch = sol['x'].full()
        lam_batch = lam_phi_batch[0:self.n_lam, :]
        phi_batch = lam_phi_batch[self.n_lam:, :]
        ...
        # solve the gradient
        dtheta_batch, dyn_loss_batch, lcp_loss_batch, = self.loss_fn(data_batch.T, lam_phi_batch, current_theta)
