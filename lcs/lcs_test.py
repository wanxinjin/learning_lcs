...
class LCS_VN:
    ...
    def diff2(self, gamma=1e-2, epsilon=1.):
        ...
        # C=np.eye(self.n_lam)
        # C = np.ones((4, 2))
        C = 1 * np.random.rand(4, self.n_lam)
        # C = 1 * np.random.randn(4, self.n_lam)
        # C = self.C
...
class LCS_PN2:
    ...
    def __init__(self, n_state, n_control, n_lam, A=None, B=None, C_r=None, dyn_offset=None,
                 D=None, E=None, G=None, H=None, lcp_offset=None, F_stiffness=1.0):
        self.n_state = n_state
        self.n_control = n_control
        self.n_lam = n_lam
        ...
        if C_r is None:
            self.C_r = SX.sym('C_r', n_state, n_lam)
            self.tunable_para += [vec(self.C_r)]
        else:
            self.C_r = DM(C_r)
        ...
        self.C = self.C_r + 0*DM(np.array([[0, 0],
                                         [0, 0],
                                         [1, 0],
                                         [0, 1]]))
        self.F = self.G @ self.G.T + F_stiffness * np.eye(self.n_lam) + self.H - self.H.T
